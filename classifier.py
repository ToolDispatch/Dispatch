...
if __name__ == "__main__":
    ...
    if not messages or should_skip(messages[-1]):
        last = args.last_task_type or "general-building"
        parts = last.rsplit("-", 1)
        domain = parts[0] if len(parts) == 2 else last
        mode = parts[1] if len(parts) == 2 else "building"
        print(json.dumps({"shift": False, "domain": domain, "mode": mode, "task_type": last, "confidence": 0.0}))
        sys.exit(0)
